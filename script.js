...
// Custom Copy Function: Sirf Name, Number, CNIC, aur Address copy karega
copyBtn.addEventListener('click', () => {
    ...
    items.forEach((item, index) => {
        // Safe key extractor (API parameters ke variations ko handle karne ke liye)
        const name = item.name || item.Name || item.full_name || 'N/A';
        const number = item.mobile || item.phone || item.number || item.phone_number || 'N/A';
        const cnic = item.cnic || item.CNIC || item.id_card || 'N/A';
